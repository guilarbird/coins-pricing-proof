"""
PHASE 1: PRICE HOLDERS MODULE
Consome dados de APIs públicas (Binance, Coinbase, ValorPro)
Normaliza para schema único com fallback automático
Usa dados simulados realistas quando APIs não estão disponíveis
"""
...
class PriceOracle:
    ...
    def _get_source_priority(self, pair: str) -> List:
        ...
        if pair == "GBPUSDT":
            return [self._binance_gbpusdt, self._coinbase_gbpusd, self._simulated_gbpusdt]
        elif pair == "USDTBRL":
            return [self._binance_usdtbrl, self._valorpro_usdtbrl, self._simulated_usdtbrl]
        elif pair == "GBPBRL":
    ...
    def _coinbase_gbpusd(self, pair: str) -> Optional[PriceHolder]:
        """Coinbase Exchange API - GBPUSD (proxy para GBPUSDT)"""
        try:
            url = "https://api.exchange.coinbase.com/products/GBP-USD/ticker"
            resp = requests.get(url, timeout=5)
            resp.raise_for_status()
            data = resp.json()
            
            # Coinbase retorna bid/ask direto
    ...
    def _valorpro_usdtbrl(self, pair: str) -> Optional[PriceHolder]:
        """ValorPro API - USDTBRL (fallback)"""
        try:
            # ValorPro é mais lento, usar como fallback
            # Placeholder: em produção, integrar com ValorPro real
            return None
        except Exception as e:
            return None
    
    def _simulated_gbpusdt(self, pair: str) -> Optional[PriceHolder]:
        """Dados simulados realistas - GBPUSDT"""
        # Baseado em cotações reais de Jan 2026
        return PriceHolder(
            pair="GBPUSDT",
            bid=1.2745,
            ask=1.2755,
    ...
    def _simulated_usdtbrl(self, pair: str) -> Optional[PriceHolder]:
        """Dados simulados realistas - USDTBRL"""
        # Baseado em cotações reais de Jan 2026
        return PriceHolder(
            pair="USDTBRL",
            bid=5.1850,
