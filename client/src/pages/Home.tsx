...
export default function Home() {
  ...
  const [coinsIofRegime, setCoinsIofRegime] = useState<'standard' | 'optimized'>('standard');
  ...
  const coinsCalc = calculateFinalAmount(gbpAmount, MARKET_MID_GBPBRL, coinsModel, undefined, coinsIofRegime);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="border border-green-900/50 bg-green-950/20 rounded-lg p-6">
              ...
              <div className="mt-6 pt-6 border-t border-green-900/30">
                <label className="text-xs font-medium mb-3 block">{t('iofRegime')}:</label>
                <div className="flex gap-2">
                  <button
                    onClick={() => setCoinsIofRegime('standard')}
                    className={`flex-1 py-2 px-3 rounded text-xs font-medium transition ${
                      coinsIofRegime === 'standard'
                        ? 'bg-green-500/20 text-green-400 border border-green-500/50'
                        : 'bg-green-900/20 text-muted-foreground border border-green-900/30 hover:bg-green-900/30'
                    }`}
                  >
                    {t('standard')} (3.5%)
                  </button>
                  <button
                    onClick={() => setCoinsIofRegime('optimized')}
                    className={`flex-1 py-2 px-3 rounded text-xs font-medium transition ${
                      coinsIofRegime === 'optimized'
                        ? 'bg-green-500/20 text-green-400 border border-green-500/50'
                        : 'bg-green-900/20 text-muted-foreground border border-green-900/30 hover:bg-green-900/30'
                    }`}
                  >
